...
def init_cloud_font():
    ...
    target_fonts = ['Noto Sans CJK JP', 'Noto Sans CJK TC', 'Noto Sans CJK SC', 'DejaVu Sans']
...
# --- [今日模式：Pandas 深度實作 P77-P90] ---
if today_mode == "📊 Pandas 數據分析 (P77-P90)" and past_mode == "--- 請選擇 ---":
    st.title("📊 Pandas 進階數據全流程實作 (P77-P90)")

    # ═══ P77: 數據載入 ═══
    try:
        # 自動檢查並建立資料夾與練習檔，確保雲端執行不報錯
        if not os.path.exists("data"): os.makedirs("data")
        if not os.path.exists("data/practice_income.csv"):
            pd.DataFrame({"年齡組": ["20-30", "30-40", "40-50"], "月薪": [35000, 52000, 68000], "獎金": [5000, 12000, 20000]}).to_csv("data/practice_income.csv", index=False, encoding="utf-8-sig")
        if not os.path.exists("data/practice_expense.csv"):
            pd.DataFrame({"年齡組": ["20-30", "30-40", "40-50"], "食衣住行支出": [15000, 20000, 25000], "娛樂教育支出": [5000, 8000, 12000]}).to_csv("data/practice_expense.csv", index=False, encoding="utf-8-sig")
        
        df_inc = pd.read_csv("data/practice_income.csv")
        df_exp = pd.read_csv("data/practice_expense.csv")
        st.sidebar.success(f"✅ 資料載入成功 (字型: {active_font})")
    ...
    with tab2:
        st.plotly_chart(px.scatter(df_final, x="月薪", y="總支出", color="年齡組"), use_container_width=True)
    with tab3:
        st.plotly_chart(px.line(df_final, x="年齡組", y=["月薪", "獎金"]), use_container_width=True)
    ...
    # 這裡移除原本寫死的「Microsoft JhengHei」，改用 init_cloud_font 偵測到的 active_font
    if active_font:
    ...
    try:
        ...
        axes[1].text(0.5, 0.5, "需聯網載入 tips 資料集", ha='center')
...
# --- [今日模式：AI 數據整合 - 鎖定 gemini-2.0-flash] ---
